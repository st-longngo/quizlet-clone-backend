...
from config.database import Post
from config.oauth2 import require_user
from serializers.post import postEntity, postListEntity
...
def create_post(post: CreatePost, user_id: str = Depends(require_user)):
...
@router.put('/{id}')
def update_post(id: str, payload: UpdatePost, user_id: str = Depends(require_user)):
